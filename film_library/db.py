...
def get_user_creator(film_id: int) -> Optional[int]:
    return Films.query.filter_by(film_id=film_id).first_or_404(description="No film with this id").user_id_added_film
...
def convert_to_dict(film, directors, genres):
    ...
    film = {"film_id": film.film_id, "user_id": film.user_id_added_film, "film_name": film.film_name,
            "description": film.description, "rating": film.rating,
            "number_of_rated_users": film.number_of_rated_users, "release_date": release_date,
            "poster_link": film.poster_link, "director_names": [], "genre_names": []}
